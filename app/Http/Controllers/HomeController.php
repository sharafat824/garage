...
class homecontroller extends Controller
{
	...
	public function dashboard()
	{
		...
		//For Service Count
		$totalService = Service::where('soft_delete', '0')->get()->count();
		$freeService = Service::where('service_type', 'free')->where('soft_delete', '0')->get()->count();
		$paidService = Service::where('service_type', 'paid')->where('soft_delete', '0')->get()->count();
		...
		if (isAdmin(Auth::User()->role_id)) {
			...
			$performance = DB::select("SELECT count(id) as count,`assign_to` as a_id FROM tbl_services where (done_status=1) and (branch_id = '" . $adminCurrentBranch->branch_id . "') and (service_date BETWEEN '" . $start_date . "' AND  '" . $end_date . "') group by `assign_to` limit 5");
		...
		if (!isAdmin(Auth::User()->role_id)) {
			if (getUsersRole(Auth::user()->role_id) == 'Customer') {
				if (Gate::allows('dashboard_owndata')) {
					//Free Service					
					$sale = Service::where([['tbl_services.done_status', '!=', 2], ['tbl_services.service_type', '=', 'free']])
				...
				$upcomingservice = Service::where([['assign_to', '=', Auth::User()->id], ['job_no', 'like', 'C%'], ['service_date', '>', $nowdate], ['soft_delete', '=', 0], ['branch_id', $currentUser->branch_id]])->take(5)->get();
			...
			$employee = User::where([['role', '=', 'employee'], ['soft_delete', '=', 0], ['branch_id', $adminCurrentBranch->branch_id]])->count();
			$Customer = User::where([['role', '=', 'Customer'], ['soft_delete', '=', 0]])->count();
			$Supplier = User::where([['role', '=', 'Supplier'], ['soft_delete', '=', 0]])->count();
			...
			$have_product = Product::where('soft_delete', '=', 0)->where('branch_id', '=', $adminCurrentBranch->branch_id)->count();
			$have_purchase = DB::table('tbl_purchases')->where('branch_id', '=', $adminCurrentBranch->branch_id)->count();
			$have_observationCount = DB::table('tbl_points')->where('soft_delete', '=', 0)->where('branch_id', '=', $adminCurrentBranch->branch_id)->count();
			...
			//Recent join customer
			$Customere = User::where([['role', '=', 'Customer'], ['soft_delete', 0]])->orderBy('id', 'desc')->take(7)->get();

			//Calendar Events 
			$serviceevent = Service::where('tbl_services.done_status', '!=', 2)->where('soft_delete', '=', 0)->where('branch_id', '=', $adminCurrentBranch->branch_id)->get();
		...
		return view('dashboard.dashboard', compact('setting', 'employee', 'Customer', 'Supplier', 'product', 'sales', 'service', 'Customere', 'sale', 'sale1', 'sale2', 'dates', 'data', 'vehical', 'performance', 'serviceevent', 'one_day', 'two_day', 'more', 'nowmonth', 'openinghours', 'holiday', 'upcomingservice', 'have_supportstaff', 'have_vehicle', 'have_product', 'have_purchase', 'have_observationCount', 'totalService', 'paidService', 'freeService'));
	}
