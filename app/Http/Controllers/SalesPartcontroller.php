...
?>
	...
	public function view(Request $request)
	{
		...
		$html = view('invoice.sales_partinvoicemodel')->with(compact('discount','viewid', 'vehicale', 'sales', 'logo', 'invioce', 'taxes', 'p_key', 'saless', 'salesp', 'salesps', 'tbl_custom_fields_salepart', 'tbl_custom_fields_customers'))->render();
	...
	public function edit($id)
	{
		...
		if (isAdmin(Auth::User()->role_id)) {
			...
			$stock = SalePart::where([['bill_no', $sales->bill_no], ['branch_id', $adminCurrentBranch->branch_id]])->get();
		} elseif (getUsersRole(Auth::user()->role_id) == 'Customer') {
			$branchDatas = Branch::get();
			$employee = DB::table('users')->where('role', '=', 'Employee')->where('soft_delete', '=', 0)->get()->toArray();
